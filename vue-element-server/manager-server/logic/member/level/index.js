...
const addLevel = async (ctx) => {
    try {
        const { levelName, packageName, packagePrice, packageDuration, benefitsList, levelIcon, linkroleId } = ctx.request.body
...
const editLevel = async (ctx) => {
    try {
        const { levelId, levelName, packageName, packagePrice, packageDuration, benefitsList, levelIcon, linkroleId, packageStatus } = ctx.request.body
        if (!levelId || !levelName || !packageName || (!packagePrice && packagePrice != 0) || !packageDuration || !benefitsList || !levelIcon || !linkroleId) {
            ctx.body = fail('缺少必要参数', CODE.PARAM_ERROR)
            return
        }
